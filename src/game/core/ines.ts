...
export const parseINes = (rom: Uint8Array): INesInfo => {
  if (rom.length < 16) {
  ...
  const chrOffset = prgOffset + prgBanks * 16_384;
...
export const decodeChrToRgba = (chr: Uint8Array, columns = 16): { width: number; height: number; rgba: Uint8Array } => {
